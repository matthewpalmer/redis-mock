...
exports.ltrim = function(mockInstance, key, start, end, callback) {
  console.log('mock instance storage', mockInstance.storage[key]);
  const trimmed = (mockInstance.storage[key].value || []).slice(start, end - start + 1);
  mockInstance.storage[key].value = trimmed;
  mockInstance._callCallback(callback, null, "OK");
};

exports.lrange = function(mockInstance, key, start, stop, callback) {
  console.log('lrange mock instance storage', mockInstance.storage[key]);
  mockInstance._callCallback(callback, null, (mockInstance.storage[key].value || []).slice(start, stop - start + 1));
}
